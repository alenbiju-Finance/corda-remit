...
Corda Remit is a modelled (not real) digital remittance provider using the local-to-local settlement model. It holds currency pools in each destination country and nets transfers against them, rather than routing payments through correspondent banking chains. This project builds the unit economics, financial model, and infrastructure cost layer for two UK outbound corridors, to test whether (and how) the opportunity differs between them.
